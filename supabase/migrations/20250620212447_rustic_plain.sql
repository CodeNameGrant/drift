/*
  # Create loan events tracking system
  ...
  1. New Tables
    - `loan_events`
      ...
      - `event_type` (enum: extra_payment, payment_skip, loan_withdrawal, interest_rate_change)
  ...
  2. Security
    ...
    - Cross-reference with debt_accounts table for ownership validation

  3. Data Validation
    - Custom trigger function validates event_data structure based on event_type
    - Automatic account balance updates after event creation
    - Immutable event design with audit trail
...
-- Note: We allow updates but rely on application logic to prevent modification of core fields
...
-- Policy for users to delete events for their own debt accounts
...
-- Function to prevent modification of core event fields (for application use)
CREATE OR REPLACE FUNCTION prevent_core_event_modification()
RETURNS TRIGGER AS $$
BEGIN
    -- Prevent modification of core event data to maintain audit trail
    IF OLD.event_type != NEW.event_type THEN
        RAISE EXCEPTION 'Cannot modify event_type after creation';
    END IF;
    
    IF OLD.event_date != NEW.event_date THEN
        RAISE EXCEPTION 'Cannot modify event_date after creation';
    END IF;
    
    IF OLD.debt_account_id != NEW.debt_account_id THEN
        RAISE EXCEPTION 'Cannot modify debt_account_id after creation';
    END IF;
    ...
    -- Allow modification of notes and limited event_data updates
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Create trigger to prevent core field modifications
CREATE TRIGGER prevent_core_event_modification_trigger
    BEFORE UPDATE ON public.loan_events
    FOR EACH ROW
    EXECUTE FUNCTION prevent_core_event_modification();
...
COMMENT ON TYPE loan_event_type IS 'Enum defining the types of events that can be recorded for loan accounts';