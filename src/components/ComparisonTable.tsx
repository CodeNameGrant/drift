...
/**
 * Comprehensive comparison table component showing active loan scenarios side by side
...
const ComparisonTable: React.FC<ComparisonTableProps> = ({ result }) => {
  ...
  // Don't show comparison table if only base scenario exists
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6" data-testid="comparison-table">
      ...
      {/* Summary Statistics - Only show active simulations */}
      {activeScenarios.length > 1 && (
        <div className="mt-8 bg-gradient-to-r from-blue-50 to-green-50 dark:from-gray-700 dark:to-gray-600 rounded-lg p-6">
          ...
          <div className={`grid gap-4 ${
            activeScenarios.length === 2 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'
          }`}>
